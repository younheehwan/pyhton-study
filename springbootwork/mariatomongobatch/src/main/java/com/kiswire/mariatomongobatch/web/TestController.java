package com.kiswire.mariatomongobatch.web;
...
import com.kiswire.mariatomongobatch.domain.maria.fda001.MariaFda001;
import com.kiswire.mariatomongobatch.domain.maria.fda001.MariaFda001Repository;
import com.kiswire.mariatomongobatch.domain.maria.fda002.MariaFda002Repository;
import com.kiswire.mariatomongobatch.domain.mongo.MongoFda;
import com.kiswire.mariatomongobatch.domain.mongo.MongoFdaRepository;
import com.kiswire.mariatomongobatch.util.CustomPickedTime;
import com.kiswire.mariatomongobatch.util.CustomTimeUtils;
...
public class TestController {
	...
	public MongoFda time() {
		...
		MongoFda mongoFda = MongoFda.builder().id(ma.getId())
				.plcR0001Timestamp(ma.getPlcR0001Timestamp()).build();
		return mongoFdaRepository.save(mongoFda);
	}
	...
	public MongoFda timePlus9Util() {
		...
		MongoFda mongoFda001 = MongoFda.builder().id(ma.getId())
				.plcR0001Timestamp(ma.getPlcR0001Timestamp()).build();
		return mongoFdaRepository.save(mongoFda001);
	}
	
	@GetMapping("/maria/fda001/minus/{minusDay}")
	public List<?> pickDay(@PathVariable int minusDay) {
	...
	public List<?> pickDayNamed(@PathVariable int minusDay) {
		// 날짜 LocalDate.now().minusDays(1) 어제
		LocalDateTime startTime = LocalDateTime.of(LocalDate.now().minusDays(minusDay), LocalTime.of(0, 0, 0));
		LocalDateTime endTime = LocalDateTime.of(LocalDate.now().minusDays(minusDay), LocalTime.of(23, 59, 59));
		Timestamp startTs = Timestamp.valueOf(startTime);
		Timestamp endTs = Timestamp.valueOf(endTime);
		return mariaFda001Repository.findByPlcR0001TimestampBetween(startTs, endTs, PageRequest.of(1, 3));
	}
	
	@GetMapping("/maria/fda001/named/util/minus/{minusDay}")
	public List<?> pickDayNamedUtil(@PathVariable int minusDay) {
		// 날짜 LocalDate.now().minusDays(1) 어제
		CustomPickedTime ts = CustomPickedTime.getStartEndTs(minusDay);
		return mariaFda001Repository.findByPlcR0001TimestampBetween(ts.getStartTs(), ts.getEndTs(), PageRequest.of(1, 3));
	}
}